...
def gen_snapshot():
    ...
    rows = fetch("""
        SELECT
            ...
            COUNT(*) FILTER (WHERE performance_today::NUMERIC > 0)   AS advancing,
            COUNT(*) FILTER (WHERE performance_today::NUMERIC < 0)   AS declining,
            ROUND(AVG(performance_today::NUMERIC), 2)                 AS avg_change_pct,
    ...
    gainers = fetch("""
        ...
        FROM latest_quotes
        WHERE performance_today IS NOT NULL
        ORDER BY performance_today::NUMERIC DESC NULLS LAST
        LIMIT 10;
    """)
    ...
    losers = fetch("""
        ...
        FROM latest_quotes
        WHERE performance_today IS NOT NULL
        ORDER BY performance_today::NUMERIC ASC NULLS LAST
        LIMIT 10;
    """)
...
def gen_sectors():
    ...
    rows = fetch("""
        SELECT
            ...
            ROUND(AVG(performance_today::NUMERIC), 2) AS avg_today,
            ROUND(AVG(performance_week::NUMERIC), 2)  AS avg_week,
            ROUND(AVG(performance_month::NUMERIC), 2) AS avg_month
        FROM latest_quotes
        WHERE major_index_membership IS NOT NULL
          AND major_index_membership != ''
          AND performance_today IS NOT NULL
        GROUP BY major_index_membership
        ORDER BY avg_today DESC NULLS LAST;
    """)
